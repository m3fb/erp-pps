...
<?php endif;?>
<p></p>
<?php if ($rueckmeldeDataProvider): ?>
<?= GridView::widget([
    'dataProvider' => $rueckmeldeDataProvider,
    ...
    'columns' => [
        ['class' => 'yii\grid\SerialColumn',
          ...
          'headerOptions'=>['style'=>'width: 10px;']
				...
				[
          'attribute' => 'Arbeitsplatz',
					'value' => function($model,$key){
						$maschine = Maschine::findOne($model->Arbeitsplatz);
						return '<input type="text" value="'.$maschine['NAME'].'" id="myMaschine_'.$key.'" readonly>
										<span class="btn btn-default glyphicon glyphicon-copy" onclick="myFunction1(`Maschine`,'.$key.')"></span>';
					},
					'format' => 'raw',
        ],
        ...
        [
          'attribute' => 'Auftrag',
					'value' => function($model,$key) {
						return '<input type="text" value="'.$model->Auftrag.'" id="myAuftrag_'.$key.'" readonly>
										<span class="btn btn-default glyphicon glyphicon-copy" onclick="myFunction1( `Auftrag` ,'.$key.')"></span>';
					},
					'format' => 'raw',
        ],
				[
          'attribute' => 'Arbeitsgang',
					'value' => function($model,$key) {
						return '<input type="text" value="'.$model->Arbeitsgang.'" id="myArbeitsgang_'.$key.'" readonly>
										<span class="btn btn-default glyphicon glyphicon-copy" onclick="myFunction1( `Arbeitsgang` ,'.$key.')"></span>';
					},
					'format' => 'raw',
        ],
        [
          'attribute' => 'Stueckzahl',
					'value' => function($model,$key) {
						Yii::$app->formatter->thousandSeparator = '';
						return '<input type="text" value="'.Yii::$app->formatter->asInteger($model->Stueckzahl).'" id="myStueckzahl_'.$key.'" readonly>
										<span class="btn btn-default glyphicon glyphicon-copy" onclick="myFunction1( `Stueckzahl` ,'.$key.')"></span>';
					},
          'format' => 'raw',
        ],
				[
          ...
          'value' => function($model,$key) {
						return Html::a('<span class="glyphicon glyphicon-ok-circle"></span>',
									...
									[
										...
										'title' => 'Rückmeldung wurde durchgeführt',
										'data-confirm' => 'Rückmeldung wurde durchgeführt? Meldung wird archiviert!',
									]
								);
					},
					'format' => 'raw',
      ...
      'panel' => [
        'heading'=>'<h3 class="panel-title"><i class="glyphicon glyphicon-inbox"></i>&nbsp;vorbereitete Rückmeldungen</h3>',
        'type'=>'success',
        'footer' => false,
				'before' =>false,
				'after' => false,
