...
public function actionTermine()
{
	...
	if (Yii::$app->request->isAjax) {
		...
		if($i == 0) {
			...
			if(($type['STATUS'] == 801) && ($row_array['title']=$type['PERSNAME'])) {
			$row_array['end'] = $type['MSTIME'];
			$row_array['title'] = $type['PERSNAME'];
			array_push($return_arr,$row_array);
			$i = 0;
			...
			else if(($type['STATUS'] == 800) && ($row_array['title'] = $type['PERSNAME'])){
			$row_array['start'] = $type['MSTIME'];
			$row_array['end'] = $row_array['end'];
			$row_array['title'] = $type['PERSNAME'];
			array_push($return_arr,$row_array);
			$i = 0;
			}
			...
			else if(($type['STATUS'] == 801) && ($row_array['title'] != $type['PERSNAME'])){
			$row_array['end'] = $type['MSTIME'];
			$row_array['title'] = $type['PERSNAME'];
			$i = 1;
			}
